...
async def get_all_orders(
    db: Client = Depends(get_db_connection), user: User = Depends(get_authorized_user)
):
    try:
        orders = await db.order.find_many(where={"order_owner_id": user.user_id})
            ...
            if cart.total_price <= user.balance:
                for prod in cart.products:
                    ...
                    await db.product.update(
                        where={"product_id": prod.product_id}, data={"available": False}
                    )
                    await db.user.update(
                        where={"user_id": user.user_id},
                        data={
                            "bought_products": {
                                "connect": [{"product_id": prod.product_id}]
                            },
                            "balance": user.balance - product_price,
                        },
                    )

                    product_shop = await db.shop.find_unique(
                        where={"shop_id": prod.owner_shop_id}
                    )

                    if product_shop:
                        product_shop_owner = await db.user.find_unique(
                            where={"user_id": product_shop.owner_id}
                        )
                        if product_shop_owner:
                            await db.user.update(
                                where={"user_id": product_shop_owner.user_id},
                                data={
                                    "balance": product_shop_owner.balance
                                    + product_price
                                },
                            )
                ...
                raise CustomGeneralException(
                    ...
                    error_msg="You do not have sufficient balance.",
                )
        else:
            raise CustomGeneralException(
                status_code=status.HTTP_403_FORBIDDEN, error_msg="Cart is empty"
            )
    except PrismaError as e:
        raise CustomPrismaException(str(e))
